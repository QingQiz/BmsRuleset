...
public class BmsStarRatingProcessorV2
{
    ...
    public BmsStarRatingResult Compute(IReadOnlyList<BmsHitObject> hitObjects, int totalColumns, int rank, double clockRate = 1.0)
    {
        ...
        smoothWl = buildWindowBounds(baseCorners, 500).left;
        ...
        // Precompute interpolation indices: linear walks instead of binary search
        var baseInterpIdx = buildInterpIdx(allCorners, baseCorners);
        var aInterpIdx = buildInterpIdx(allCorners, aCorners);

        jbar = interpValuesFast(allCorners, baseCorners, baseInterpIdx, jbar);
        xbar = interpValuesFast(allCorners, baseCorners, baseInterpIdx, xbar);
        pbar = interpValuesFast(allCorners, baseCorners, baseInterpIdx, pbar);
        abar = interpValuesFast(allCorners, aCorners, aInterpIdx, abar);
        rbar = interpValuesFast(allCorners, baseCorners, baseInterpIdx, rbar);
        cArr = stepInterpFast(allCorners, baseInterpIdx, cArr);
        ksArr = stepInterpFast(allCorners, baseInterpIdx, ksArr);
    ...
    /// </summary>
    private static (int[] left, int[] right) buildWindowBounds(double[] x, double window)
    {
        var left = new int[x.Length];
        var right = new int[x.Length];
        int l = 0, r = 0;
        for (var i = 0; i < x.Length; i++)
        {
    ...
    /// <summary>Linear interpolation using a precomputed index from buildInterpIdx.</summary>
    private static double[] interpValuesFast(double[] newX, double[] oldX, int[] interpIdx, double[] oldVals)
    {
        var result = new double[newX.Length];
        for (var i = 0; i < newX.Length; i++)
        {
    ...
    /// <summary>Step interpolation (zero-order hold) using a precomputed index.</summary>
    private static double[] stepInterpFast(double[] newX, int[] interpIdx, double[] oldVals)
    {
        var result = new double[newX.Length];
        for (var i = 0; i < newX.Length; i++)
        {
    ...
    private static int searchSortedLeft(IReadOnlyList<(int column, double head, double tail)> list, double value)
    {
    ...
    private void preprocessFile(IReadOnlyList<BmsHitObject> hitObjects, int rank, double clockRate)
    {
        ...
        TotalTimeT = Math.Max(
            noteSeq.Count > 0 ? noteSeq[^1].head : 0,
            tailSeq.Count > 0 ? tailSeq[^1].tail : 0
        ) + 1;
    }
    ...
    private double[][] getKeyUsage400()
    {
        ...
        foreach (var (k, head, tail) in noteSeq)
        {
            ...
            for (var i = left400; i < left && i >= 0 && i < baseCorners.Length; i++)
                usage[k][i] += 3.75 - 3.75 / 160000.0 * (baseCorners[i] - start) * (baseCorners[i] - start);

            for (var i = right; i < right400 && i < baseCorners.Length; i++)
                usage[k][i] += 3.75 - 3.75 / 160000.0 * (baseCorners[i] - end) * (baseCorners[i] - end);
        }
    ...
    private double[] computeRbar()
    {
        ...
        for (var i = 0; i < tailSeq.Count; i++)
        {
            ...
            var colNotes = noteSeqByColumn[k];
            var idx = searchSortedLeft(colNotes, hi);
            var hNext = idx + 1 < colNotes.Count ? colNotes[idx + 1].head : 1e9;
    ...
    private void computeCAndKs(out double[] cArr, out double[] ksArr)
    {
        ...
        var noteHitTimes = noteSeq.Select(n => n.head).ToArray();
    ...
    private double computeStarRating(double[] d, double[] cArr)
    {
        ...
        // Sort D values in ascending order, reordering weights in parallel
        var dSorted = (double[])d.Clone();
        var wSorted = (double[])effectiveWeights.Clone();
        Array.Sort(dSorted, wSorted);
