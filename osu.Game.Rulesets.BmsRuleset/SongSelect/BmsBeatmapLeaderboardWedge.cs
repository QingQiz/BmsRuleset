...
public partial class BmsBeatmapLeaderboardWedge : VisibilityContainer
{
    ...
    private readonly List<ScheduledDelegate> scoreSfxDelegates = [];
    ...
    {
        var scores = Scope.Value == BeatmapLeaderboardScope.Local ? fetchedScores.Value : leaderboardManager.Scores.Value;
        ...
        if (Scope.Value != BeatmapLeaderboardScope.Local && leaderboardManager.CurrentCriteria?.Beatmap?.Equals(beatmap.Value.BeatmapInfo) != true)
            return;
    ...
    protected void SetScores(IEnumerable<ScoreInfo> scores, ScoreInfo? userScore = null, int? totalCount = null)
    {
        ...
        }), loadedScores =>
        {
            ...
            foreach (var d in loadedScores)
            {
                ...
                var visible = d.ScreenSpaceDrawQuad.TopLeft.Y < d.Parent!.ChildMaskingBounds.BottomLeft.Y;
    ...
    protected override void UpdateAfterChildren()
    {
        ...
        {
            var topY = c.ToSpaceOfOtherDrawable(Vector2.Zero, scoresContainer).Y;
            var bottomY = topY + height;

            var requireBottomFade = bottomY >= fadeBottom;
            var requireTopFade = topY < fadeTop;
