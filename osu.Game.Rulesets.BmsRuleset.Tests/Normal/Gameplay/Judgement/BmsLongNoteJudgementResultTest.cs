...
public class BmsLongNoteJudgementResultTest
{
    ...
    public void TestDrawableResultAcceptsEndpointsOnlyOnce()
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(result.EndpointResults, Is.EqualTo([endpoint]));
            Assert.Throws<InvalidOperationException>(() => result.SetEndpointResults([endpoint]));
        });
    }
    ...
    public void TestDrawableResultAcceptsReplayedEndpointsAfterFrameworkReset()
    {
        ...
        Assert.That(result.EndpointResults, Is.EqualTo([replayed]));
    }
    ...
    public void TestRejectsDuplicateEndpointKinds()
    {
        ...
        Assert.Throws<ArgumentException>(() => _ = new BmsLongNoteJudgementResult(source, source.CreateJudgement(), endpoints));
    }
