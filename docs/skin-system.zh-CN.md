...
判定弹窗的位置和大小应通过编辑器控制，图案和动画则通过皮肤图像文件修改。
...
- **目标**：个人最好等级的下一个等级所需的最低 EX Score，依次为 C、B、A、AA、AAA、S；个人最好已经达到 S 时，目标
  仍为 S。
...
Class、ExClass、ExHard Class 分别使用 Hard、ExHard、Hazard 的填充颜色。调整组件大小只会改变血条的可见宽度和高度，
不会修改血量、阈值或血条机制。
