...
选择算法适用于普通音符和长音符头部，也适用于皿轨。每个候选音符使用其布局、RANK/EXRANK 及判定窗口 Mod
共同决定的窗口。长音符松键仍沿用原有规则；E-POOR 仍不消耗音符。

每局开始时会固定算法，因此更改设置从下一局生效。新回放会保存当局算法，播放时不受当前设置影响。
此设置加入之前录制的旧回放继续使用原有的判定选择逻辑。
...
| 判定选择算法 | 连击优先（LR2） | 连击优先（LR2）、时间差优先（AC）、最早音符优先、分数优先 | 决定同一轨道的判定窗口重叠时判定哪个音符。更改从下一局生效，回放保留录制时的算法。详见[选择规则](#判定选择算法)。 |
