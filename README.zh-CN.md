...
要删除所有已导入的 BMS 内容，使用同一设置栏目中的 **"Delete all imported BMS files"** 按钮。
...
| 字段                | 命令                                                         | 说明                                                                              |
|----------------------|-------------------------------------------------------------|-----------------------------------------------------------------------------------|
| 标题、艺术家、流派   | `#TITLE`、`#ARTIST`、`#GENRE`                               |
| Play level           | `#PLAYLEVEL`（作为显示标签追加到难度名）                    |                                                                                   |
| 初始 BPM             | `#BPM`                                                      |                                                                                   |
| 扩展 BPM 表          | `#BPMxx`                                                    |                                                                                   |
| STOP 表              | `#STOPxx`                                                   |                                                                                   |
| 采样定义             | `#WAVxx`                                                    |                                                                                   |
| 长音符类型           | `#LNTYPE 1` / `#LNTYPE 2` / `#LNOBJ`                        |                                                                                   |
| 判定等级             | `#RANK`（0–4，影响判定窗口）                                |                                                                                   |
| 血量总量             | `#TOTAL`                                                    |                                                                                   |
| 玩家模式             | `#PLAYER`                                                   | **已忽略** — 布局仅根据通道存在和文件扩展名推断                                   |
| 小节长度             | 通道 `02`                                                   |                                                                                   |
| 文本事件             | `#TEXTxx`、`#SONGxx`、通道 `99`                             |                                                                                   |
| Random               | `#IF`、`#SWITCH` 及相关命令                                 | 游戏支持，但不同分支的元数据显示尚未实现                                          |
...
**未解析/未实现：** `#EXRANK`、`#SUBTITLE`、`#STAGEFILE`、`#BANNER`、BGA/图像通道（`04`、`06`、`07` 等）。
...
每个表同时创建一个名为 `"BMS Table: {表名}"` 的 **BeatmapCollection**，包含所有匹配的谱面。可以直接在选歌界面的收藏夹列表中浏览。
...
点击 **Subdivide** 将表的收藏夹拆分为每级独立的收藏夹（例如 `"BMS Table: Table ★1"`、`"BMS Table: Table ★2"`）。
点击 **Unsubdivide** 合并回去。
...
| **解析器**   | `#SUBTITLE`、`#SUBARTIST`、`#STAGEFILE`、`#BANNER`、`#BACKBMP`、`#MOVIE` — 仅元数据         |
| **解析器**   | `#PATH_WAV` / `#PATH_BMP` — 资源路径前缀                                                    |
| **解析器**   | `#EXWAVxx`、`#WAVCMD`、`#VOLWAV` — 高级音频控制                                            |
| **解析器**   | `#STP` — 绝对 STOP 序列                                                                     |
| **解析器**   | `#DEFEXRANK`、`#BASEBPM`、`#MAKER`、`%URL`、`%EMAIL`、`#COMMENT`、`#DIFFICULTY` — 元数据     |
