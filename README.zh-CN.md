...
调整音符到达判定线的画面时机，而不改变音频、判定时间、计分或 keysound 播放。正视觉偏移会让音符提前显示，
适合 Slow 较多的玩家；负值会让音符延后显示，适合 Fast 较多的玩家。

有效的本地游玩会根据击打误差中位数生成视觉偏移建议。BMS 设置会显示最近的建议值，可以手动应用其平均值，
也可以在每次游玩后自动应用新建议。Replay、自动游玩以及有效时机判定少于 50 个的游玩不会参与校准。

同一 BMS 设置栏目还提供 **LN 尾视觉偏移**。它只会让长按音符的尾部提前显示，使 LN 看起来更短。

### BMS 帧率解锁

在 BMS 设置中开启**解除帧率上限**，可以解除 BMS 游玩时渲染、更新与输入轮询的 1000 Hz 上限。

**副作用：** 更大的 GC 和 GPU 压力可能造成额外卡顿，遇到时请关闭此选项。
...
官方 osu!mania 7K 谱面可使用 BMS 游玩。7 条轨道映射到 BME 键盘列，皿列留空；音符时间、长音符、BPM、拍号、
offset 和滚速变化均会保留。其它键数暂不支持。

要启用转谱此功能，选择 BMS ruleset并在选歌界面的筛选面板中开启 **显示转谱**。
